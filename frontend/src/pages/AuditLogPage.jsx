...
function Badge({ op }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-mono font-semibold ${style.badge}`}
    >
...
export default function AuditLogPage() {
  ...
  const fetchLogs = useCallback(async () => {
    ...
    try {
      ...
      // Derive filter options from first load
      if (data.meta) {
          ...
          <div className="flex flex-col gap-1 min-w-[160px]">
            ...
            {tables.length > 0 ? (
              <select
                ...
                onChange={(e) =>
                  setFilters((f) => ({ ...f, table_name: e.target.value }))
                }
                className="bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500"
              >
...
function AuditRow({ log }) {
  ...
  const hasChanges =
    log.old_values || log.new_values || log.changed_fields?.length;
    ...
    <>
      ...
      >
        <td className="px-4 py-3 text-slate-300 font-mono text-xs whitespace-nowrap">
          {formatTs(log.created_at || log.timestamp || log.ts)}
        </td>
        <td className="px-4 py-3">
          <span className="text-slate-200">{log.user_email || log.user || log.changed_by || "—"}</span>
        </td>
        <td className="px-4 py-3">
          <code className="text-xs text-sky-400 bg-sky-400/10 px-2 py-0.5 rounded">
            {log.table_name || log.tablename || "—"}
          </code>
        </td>
        <td className="px-4 py-3">
          <Badge op={log.operation || log.action || "—"} />
        </td>
        <td className="px-4 py-3 font-mono text-xs text-slate-400">
          {log.record_id || log.row_id || log.id_affected || "—"}
        </td>
        <td className="px-4 py-3">
          {hasChanges ? (
...
function DiffView({ oldValues, newValues, fields }) {
  const parseJ = (v) => {
    if (!v) return {};
  ...
  return (
    <div className="rounded-lg overflow-hidden border border-slate-700/50 text-xs font-mono">
      <div className="grid grid-cols-2 divide-x divide-slate-700/50">
        <div>
          <div className="px-3 py-1.5 bg-red-500/10 text-red-400 font-semibold text-[10px] uppercase tracking-wider border-b border-slate-700/50">
            Before
          </div>
          {keys.map((k) => (
            <div
              key={k}
              className="flex px-3 py-1.5 border-b border-slate-800/50 last:border-0"
            >
              <span className="text-slate-500 w-28 shrink-0">{k}</span>
              <span
                className={
                  oldObj[k] !== newObj[k] ? "text-red-300" : "text-slate-400"
                }
              >
        ...
        <div>
          <div className="px-3 py-1.5 bg-emerald-500/10 text-emerald-400 font-semibold text-[10px] uppercase tracking-wider border-b border-slate-700/50">
            After
          </div>
          {keys.map((k) => (
            <div
            ...
            >
              ...
              <span
                className={
                  oldObj[k] !== newObj[k] ? "text-emerald-300" : "text-slate-400"
                }
              >
...
function generateMock() {
  ...
  const users = [
    "admin@infralock.dev",
    "aakash@infralock.dev",
    "shrestha@infralock.dev",
    "ops@infralock.dev",
  ];
  return Array.from({ length: 120 }, (_, i) => {
    ...
    return {
      ...
      old_values:
        op !== "INSERT"
          ? JSON.stringify({ status: "active", updated_at: "2025-01-01" })
          : null,
      new_values:
        op !== "DELETE"
          ? JSON.stringify({ status: "inactive", updated_at: new Date().toISOString() })
          : null,
      changed_fields: op === "UPDATE" ? ["status", "updated_at"] : null,
    };
  });
}
